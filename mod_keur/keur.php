	...
	?>
		<section>
			...
			<ul class="breadcrumb" style="margin-bottom:5px;">
				<li class="active">Data Perp. KEUR</li>
			</ul>
			<div class="control-group pull-left">
				<button class="btn btn-info" type="button" onclick="window.location='media.php?module=keur&&act=tambah'"><i class="icon-chevron-right icon-white"></i> Input Perpanjangan</button>
			</div>
				...
				<div class="span12 pull-left">
					...
					<div id="tabel_awal">
						<table class="table table-striped">
							...
							<tbody>
							<?php
								$query = mysqli_query($conn, "SELECT * FROM perp_keur, mobil, pegawai WHERE perp_keur.id_mobil=mobil.id_mobil AND mobil.id_peg=pegawai.id_peg ORDER BY id_keur DESC LIMIT $posisi, $batas");
								$no = $posisi+1;
								while ($r = mysqli_fetch_array($query)) {
							?>
								<tr>
									<td><?php echo $no.".";?></td>
									<td><?php echo $r['kode_perp_keur'];?></td>
									...
									<td>
										<button class="btn btn-warning" onclick="window.location='media.php?module=keur&&act=cost_real&&kode_perp_keur=<?php echo $r['kode_perp_keur'] ?>'" <?php if ($r['status'] != 'APPROVED_2') { ?> disabled <?php } ?>>Input Realisasi Biaya</button>
									</td>
									<?php } else { ?>
									...
									<td>
										<div class="btn-group">
											...
											<ul class="dropdown-menu">
												...
												<li><a href="<?php echo "$aksi?module=approve_1&&kode_perp_keur=$r[kode_perp_keur]&&kodeuser=$kodeuser";?>" onclick="return confirm('Apakah anda yakin, menyetujui perpanjangan KEUR <?php echo $r['kode_perp_keur'] ?> ?')"><i class="icon-pencil"></i> Approve_1</a></li>
												<?php } ?>

												<?php if (($r['status'] == 'APPROVED_1') && ($akses == 3) && ($r['cost_est'] > 0)) { ?>
												<li><a href="<?php echo "$aksi?module=approve_2&&kode_perp_keur=$r[kode_perp_keur]&&kodeuser=$kodeuser";?>" onclick="return confirm('Apakah anda yakin, menyetujui perpanjangan KEUR <?php echo $r['kode_perp_keur'] ?> ?')"><i class="icon-pencil"></i> Approve_2</a></li>
												<?php } ?>

												<?php if ($akses == 3) { ?>
												<li><a href="<?php echo "$aksi?module=hapus&&kode_perp_keur=$r[kode_perp_keur]";?>" onclick="return confirm('Apakah anda yakin, ingin menghapus data perpanjangan KEUR <?php echo $r['kode_perp_keur'];?> ?')"><i class="icon-trash"></i> Delete</a></li>
												<?php } ?>
												
												<?php if ($r['status'] == 'APPROVED_2') { ?>
												<li><a href="voucher/v_keur.php?kode_perp_keur=<?php echo $r['kode_perp_keur'] ?>" onclick="window.location='voucher/v_keur.php?kode_perp_keur=<?php echo $r['kode_perp_keur'] ?>"><i class="icon-print"></i> Print</a></li>
												<?php } ?>

												<?php if (($r['status'] == 'APPROVED_2') && (($akses == 3) || ($akses == 5))) { ?>
												<li><a href="<?php echo "$aksi?module=close&&kode_perp_keur=$r[kode_perp_keur]" ?>" onclick="return confirm('Apakah anda yakin, ingin menutup data perpanjangan KEUR <?php echo $r['kode_perp_keur'];?> ?')"><i class="icon-remove"></i> Close</a></li>
												<?php } ?>
											</ul>
										</div>
									</td>
							...
							?>
								<tr>
									<td colspan="9">
									<?php
										$jmldata = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM perp_keur"));
...
<?php
	break;
		...
		$nounik = "KEUR-".$acak1.$acak2;
?>
		<section>
			<ul class="breadcrumb" style="margin-bottom:5px;">
				<li><a href="media.php?module=keur">Data Perp. KEUR</a> <span class="divider"><b>&gt;</b></span></li>
				<li class="active">Input Perpanjangan KEUR</li>
			</ul>
			...
			<div class="row-fluid">
				<div class="span12 pull-left">
					<form method="post" enctype="multipart/form-data" action="<?php echo "$aksi?module=tambah";?>">
						...
						<fieldset>
							<legend class="span7 offset1">Input Perp. KEUR</legend>
							<div class="clear"></div>
							<div class="span3 offset1" style="border:1px solid #fff; border-radius:5px; padding:10px; background:#54c7dc">
								<div class="control-group">
									<label class="control-label" for="inputPassword">Kode Perp. KEUR</label>
									<div class="controls">
										<input class="span6" type="text" value="<?php echo $nounik;?>" disabled></input>
										<input class="span6" type="hidden" value="<?php echo $nounik;?>" name="kode_perp_keur"></input>
									</div>
								</div>
								<div class="control-group">
...
<?php
	break;
		...
		$query = mysqli_query($conn, "SELECT * FROM perp_keur WHERE perp_keur.kode_perp_keur='$kode_perp_keur'");
		$row = mysqli_fetch_array($query);
?>
		<section>
			<ul class="breadcrumb" style="margin-bottom: 5px">
				<li><a href="media.php?module=keur">Data Perp. KEUR</a> <span class="divider"><b>&gt;</b></span></li>
				<li class="active">Input Realisasi Biaya Perpanjangan KEUR</li>
			</ul>
			...
			<div class="row-fluid">
				<div class="span12 pull-left">
					<form method="post" enctype="multipart/form-data" action="<?php echo "$aksi?module=cost_real" ?>">
						<fieldset>
							<legend class="span7 offset1">Input Realisasi Biaya Perp. KEUR</legend>
							<div class="clear"></div>
							...
							<div class="span3 offset1" style="border:1px solid #fff; border-radius:5px; padding:10px; background:#54c7dc">
								<div class="control-group">
									<label class="control-label" for="inputPassword">Kode Perp. KEUR</label>
									<div class="controls">
										<input class="span6" type="text" id="inputText" value="<?php echo $row['kode_perp_keur']; ?>" disabled></input>
										<input class="span6" type="hidden" id="inputText" value="<?php echo $row['kode_perp_keur']; ?>" name="kode_perp_keur"></input>
									</div>
								</div>
